...
export class CLAnimationHelpers {
   ...
   */
  static binaryFlashRandomInterval(
    source,
    speed,
    dt,
    offPhaseDelay = 10,
    timeRangeFlipped: [start: number, end: number] = [1, 1],
  ) {
    // Ensure the array is valid
    if (
      timeRangeFlipped.length != 2 ||
      timeRangeFlipped[0] < 1 ||
      timeRangeFlipped[1] < 1 ||
      timeRangeFlipped[0] > timeRangeFlipped[1]
    ) {
      console.error(
        'CommunityLighting error: binaryRandomInterval called with invalid timeRangeFlipped: ' + timeRangeFlipped,
      );
      return;
    }
    ...
    if (!source._flipped) {
      ...
      if (source._currentTime >= offPhaseDelay) {
        ...
        if (Math.random() > 0.9 - 0.08 * speed) {
          ...
          source._revertTime = Math.floor(
            Math.random() * (timeRangeFlipped[1] - timeRangeFlipped[0] + 1) + timeRangeFlipped[0],
          );
        }
      }
   ...
   * @param {Array<number>} [timeRangeFlipped=[1,1]] - Range of time that the phase can remain flipped 'on'. Both elements must be >= 1, and timeRangeFlipped[1] must be >= timeRangeFlipped[0]
   */
  static ternaryFlashRandomInterval(
    source,
    speed,
    dt,
    offPhaseDelay = 10,
    ...
    if (
      ...
      console.error(
        'CommunityLighting error: ternaryRandomInterval called with invalid timeRangeFlipped: ' + timeRangeFlipped,
      );
      return;
    }
