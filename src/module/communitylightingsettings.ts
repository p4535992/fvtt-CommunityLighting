...
export class CommunityLightingSettings extends FormApplication {
    ...
    async getData() {
        const data:any = {};
        const src = ROUTE_PREFIX?`/${ROUTE_PREFIX}/modules/${CONSTANTS.MODULE_NAME}/assets/lights.json`:`/modules/${CONSTANTS.MODULE_NAME}/assets/lights.json`;
        const resp = await fetch(src);
        if (resp.status !== 200) {
            console.warn(`Unable to load community lighting file: ${src}`);
        }
        const tempAuthorsObj = await resp.json().then(json => {
