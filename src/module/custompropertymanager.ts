...
export class CLCustomPropertyManager {
  static async updateCustomProperties(updateData, lightAnimationChanges) {
    ...
    const placeableObject: Token | AmbientLight | undefined =
      canvas.lighting?.get(updateData.id) ?? canvas.tokens?.get(updateData.id);
  ...
  static async saveCustomProperties(data, flagsChanges) {
    ...
    if (customProperties) {
      ...
      if (data._source.actor) {
        data._source.update(
          { lightAnimation: customProperties.pointSource._source.data.lightAnimation },
          { diff: false, loadedProperty: true },
        );
      }
    }
  }
  ...
  static loadCustomProperties(pointSource) {
    ...
    if (customProperties) {
      ...
      if (pointSource.object.data._source.actor) {
        pointSource.object.data._source.update(
          { lightAnimation: pointSource._source.lightAnimation },
          { diff: false, loadedProperty: true },
        );
      }
    }
  ...
  static async addCustomProperties(
    objectConfig,
    customPropertySibling,
    ...
    function insertDescription(customPropertyObject) {
      return customPropertyObject.description
        ? `<p class="hint community-lighting-custom-property-noanim">${customPropertyObject.description}</p>`
        : '';
    }
      ...
      switch (customPropertyObject.type) {
        ...
        case 'color': {
          const customPropertyEl = $(
            `<div class="form-group community-lighting-custom-property">
                            ...
                            <input class="color" type="text" name="lightAnimation.${
                              customPropertyObject.varName
                            }" value="${currentValue}">
                            <input type="color" value="${currentValue}" data-edit="lightAnimation.${
              customPropertyObject.varName
            }">
          ...
          const customPropertyEl = $(
            `<div class="form-group community-lighting-custom-property">
                        ...
                        <div class="form-fields">
                            <input type="range" name="lightAnimation.${
                              customPropertyObject.varName
                            }" value="${currentValue}" min="${customPropertyObject.min}" max="${
              customPropertyObject.max
            }" step="${customPropertyObject.step}" data-dtype="Number">
        ...
        case 'checkbox': {
          const customPropertyEl = $(
            `<div class="form-group community-lighting-custom-property">
                        <label>${customPropertyObject.title}</label>
                        <input type="checkbox" name="lightAnimation.${
                          customPropertyObject.varName
                        }" data-dtype="Boolean" ${currentValue ? 'checked' : ''}>
                        ${insertDescription(customPropertyObject)}
                    </div>`,
          );
          if (animateShow) {
        ...
        case 'select': {
          ...
          customPropertyObject.options.forEach((option) => {
            options += `<option value="${option.value}"${option.value === currentValue ? 'selected' : ''}>${
              option.label
            }</option>`;
          });
          const customPropertyEl = $(
  ...
  static async addOptGroups(html) {
    ...
    html.find("[value$='CommunityLightingSeparatorStart']").each((index, element) => {
      html
        .find(`[value='${element.innerText}CommunityLightingSeparatorStart']`)
        .nextUntil(`[value='${element.innerText}CommunityLightingSeparatorEnd']`)
        .wrapAll(`<optgroup label="${element.innerText}"></optgroup>`);
    });
  ...
  static async onRenderTokenOrLightConfig(objectConfig, html, data) {
    ...
    if (customAnimationProperties && customAnimationProperties.length > 0) {
      CLCustomPropertyManager.addCustomProperties(
        objectConfig.object,
        customPropertySibling,
        customAnimationProperties,
      );
    ...
    animTypeSelector.on('change', function () {
      ...
      if (customAnimationProperties && customAnimationProperties.length > 0) {
        CLCustomPropertyManager.addCustomProperties(
          objectConfig.object,
          customPropertySibling,
          (<any>CONFIG.Canvas.lightAnimations[this.value]).customProperties,
          true,
        );
      }
    });
  ...
  static onUpdateLightOrToken(doc, changes, diff, user) {
    if (!changes?.flags?.CommunityLighting) {
      if (
        doc.documentName === 'AmbientLight' &&
        doc.sheet._state === DocumentSheet.RENDER_STATES.RENDERING &&
        game.settings.get(CONSTANTS.MODULE_NAME, 'closeLightOnSubmit') === false
      ) {
        ui.notifications.notify(game.i18n.localize('COMMUNITYLIGHTING.notif.lightUpdated'));
      } else if (
        doc.documentName === 'Token' &&
        changes.lightAnimation &&
        game.settings.get(CONSTANTS.MODULE_NAME, 'closeTokenOnSubmit') === false
      ) {
