...
export class CLAnimationManager {
  ...
  async getCommunityLights() {
    const src = ROUTE_PREFIX
      ? `/${ROUTE_PREFIX}/modules/CommunityLighting/lights.json`
      : '/modules/CommunityLighting/lights.json';
      ...
      for (const author in customLightsObject) {
        ...
        lightsArray?.forEach((light) => {
          animations[`${author}${light.name}`] = {
            ...
            illuminationShader:
              CLAnimationManager.getShaderClass(light.shaders?.illumination) || AdaptiveLightingShader, //StandardIlluminationShader,
