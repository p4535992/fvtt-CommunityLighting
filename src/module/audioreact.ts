export class CLAudioReactor {
  ...
  static getEZFreqPower(index, soundBoard = false, channel = 0, special) {
    ...
    return soundBoard
      //@ts-ignore
      ? CLAudioReactor.soundboardFreqPowers[channel][index] || 0
      //@ts-ignore
      : CLAudioReactor.freqPowers[channel][index] || 0;
  }

  static getAudioFreqPower(
    pointSource,
    currentPeak,
    smoothing = 5,
    band = [0, 63],
  ...
  static initSoundboardAnalyser() {
    ...
    if (game.audio.soundboardGain) {
      ...
      CLAudioReactor.soundboardSampleBuffer = [
  ...
  static attemptEmbiggen = (sound, retryCount = 0) => {
    ...
    if (!sound.node || !sound.node.buffer) {
      ...
      setTimeout(() => {
        this.attemptEmbiggen(sound, ++retryCount);
      }, 500);
      return;
    }
    if (!sound.container.sourceNode._clConnected) {
      ...
      setTimeout(() => {
        CLAudioReactor.attemptEmbiggen(sound, ++retryCount);
      }, 500);
    }
  };
}
