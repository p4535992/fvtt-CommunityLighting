 ...
 *
 */
export class CLAnimations extends LightSource {
  constructor() {
    super();
  ...
  blitzShaderTest(dt, { speed = 5, intensity = 5 }) {
  ...
  blitzStaticBlur(dt, { speed = 5, intensity = 5, blurStrength = 20 }) {
    CLAnimationHelpers.addIlluminationBlur(this, blurStrength);
  }

  blitzFadeSimple(dt, { speed = 5, intensity = 5 }) {
  ...
  blitzLightningSimple(dt, { speed = 5, intensity = 5 }) {
  ...
  blitzElectricFault(dt, { speed = 5, intensity = 5 }) {
    // Use binaryRandomInterval to flip on at random
    // light, speed, delay before attempting to flip, frame-range light can remain flipped 'on'
    CLAnimationHelpers.binaryFlashRandomInterval(this, speed, dt, 50, [1, 10]);
  ...
  blitzTorch(
    ...
    if (t - this._animTime < targetMS) {
      const alteredValue = Math.random() * 0.001;
      if (this._flipped) {
        iu.ratio -= alteredValue;
        iu.alpha -= alteredValue;
        cu.alpha -= alteredValue;
        if (this._originalColor && secondaryColor) {
          const colorScale = chroma.scale([this._originalColor, secondaryColor]).domain([0, 1]); // Get a color between original, secondaryColor and tertiaryColor, mapped from 0 to 1
  ...
  blitzSimpleFlash(dt, { speed = 5, intensity = 5 }) {
    CLAnimationHelpers.binaryTimer(this, speed, dt);
  ...
  blitzRGBFlash(dt, { speed = 5, intensity = 5 }) {
  ...
  blitzPoliceFlash(dt, { speed = 5, intensity = 5 }) {
    // Track _flipped value at start of cycle
    const flipped = this._flipped;
    CLAnimationHelpers.forceColorationShader(this, '#ff0000');
    CLAnimationHelpers.includeAnimation(this, 'blitzSimpleFlash', dt, { speed, intensity }); // Run blitzSimpleFlash
  ...
  // EXPERIMENTAL
  blitzFadeMusic(dt, { speed = 5, intensity = 5, listeningBand = 'mid', gainNode = 'master', exponent = 1 }) {
  ...
  blitzPulseMusic(dt, { speed = 5, intensity = 5, listeningBand = 'mid', gainNode = 'master', exponent = 1 }) {
  ...
  blitzPulseMusicColorshift(
    ...
    if (this._originalColor && secondaryColor && tertiaryColor) {
      const colorScale = chroma.scale([this._originalColor, secondaryColor, tertiaryColor]).domain([0, 1]); // Get a color between original, secondaryColor and tertiaryColor, mapped from 0 to 1
  ...
  blitzForceFieldExtension(
    dt,
    { speed = 5, intensity = 5, secondaryColor = '#00ff00', useColorSpeed = true, colorSpeed = 5, colorSpeedMult = 1 },
  ) {
    if (this._placeableType == 'AmbientLight') {
      this._originalColorAlpha = this?._source?.data?.tintAlpha;
      this._originalColor = this?._source?.data?.tintColor;
    ...
    if (this._originalColor && secondaryColor) {
      const colorScale = chroma.scale([this._originalColor, secondaryColor]).domain([0, 1]); // Get a color between original and secondaryColor, mapped from 0 to 1
      this.coloration.uniforms.color = hexToRGB(colorScale(this._wave.simplifiedValue).num()); // Set color to a color from colorScale, using full intensity cos wave to get a smooth 0 to 1 and back
    }
  }
  blitzPolyLight(
    ...
    if (t - this._animTime < targetMS) {
      const alteredValue = Math.random() * 0.001 * intensity;
      if (this._flipped) {
        if (shouldFlicker) {
          iu.ratio -= alteredValue;
        }
  ...
  customFire(
    ...
    if (!this._updateHook) {
      ...
      Hooks.on('updateAmbientLight', (updateData: LightData, options) => {
        if (this._updateHook) {
          return this._updateHook(this, updateData, options);
        }
      });
      // Run on first enable
    ...
    if (audioReactive) {
      const peak = Math.pow(CLAnimationHelpers.getEzFreqPower(listeningBand, gainNode == 'soundboard'), exponent);
      iu.musicWave = peak;
      cu.musicWave = peak;
    } else {
      iu.musicWave = 1.0;
    ...
    const rampUniformVal = this._ramp ? Circ.easeIn(this._rampValue) : 1.0;
    ...
    if (this._lastUpdate - this._circleAnimTime < targetMS) {
      const alteredValue = Math.random() * 0.001 * circleIntensity;
      if (this._flipped) {
        if (shouldFlicker) {
          iu.ratio -= alteredValue;
        }
  ...
  blitzForgottenAdventuresCustom(
    dt,
    {
      speed = 5,
      intensity = 5,
    ...
    if (shouldFlicker) {
      CLAnimationHelpers.includeAnimation(this, 'blitzTorch', dt, {
        ...
        intensity,
        blurStrength,
        alterAlpha: shouldFlickerAlpha,
        alterTranslation: shouldFlickerTranslate,
        secondaryColor:
          this._placeableType == 'AmbientLight' ? this?._source?.data?.tintColor : this?._source?.data?.lightColor,
      });
    } else {
  ...
  /* Author: SecretFire */
  secretFireAnimateTimeForceColor(dt, { speed = 5, intensity = 5 }) {
  ...
  secretFireAnimateStarLight(dt, { speed = 5, intensity = 5, secondaryColor = '#00ff00' }) {
    CLAnimationHelpers.forceColorationShader(this, '#ff0000'); // Force the lights tintColor to Red if the user has not set one
    CLAnimationHelpers.includeAnimation(this, 'foundryTime', dt, { speed, intensity }); // Call `foundryTime` to manipulate the custom shaders based on time
  ...
  secretFireAnimateStarLightMusic(
    dt,
    { speed = 5, intensity = 5, secondaryColor = '#00ff00', listeningBand = 'mid', gainNode = 'master', exponent = 1 },
  ) {
    if (!this._currentPeak) {
      this._currentPeak = 0; // store currentPeak inside the pointSource
    }
